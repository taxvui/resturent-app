...
@RoutePage()
class DepartmentListView extends ConsumerStatefulWidget {
  const DepartmentListView({super.key});

  @override
  ConsumerState<DepartmentListView> createState() => _DepartmentListViewState();
}

class _DepartmentListViewState extends ConsumerState<DepartmentListView>
    with PaginatedControllerMixin<DepartmentModel> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: CustomAppBar(
        title: Text(context.t.hrm.pageTitles.department),
      ),
      body: PermissionGate(
        moduleKey: PMKeys.department,
                        ...
                        onEdit: ref.canT(
                          ...
                          input: () async {
                            return context.router.push<void>(
                              ManageDepartmentRoute(editModel: item),
                            );
                          },
                        ),
                        onDelete: ref.canT(
                      ...
                      return EmptyWidget(
                        ...
                        emptyBuilder: (context) {
                          return RetryButtons.scrollView(
                            context.t.hrm.emptyStates.noDepartments,
      ...
      floatingActionButton: SizedBox(
        ...
        child: FloatingActionButton.extended(
          ...
          label: Text(context.t.hrm.pageTitles.addDepartment),
          icon: const Icon(Icons.add, size: 18),
        ),
      ).can(PMKeys.department, action: PermissionAction.create),
    ).unfocusPrimary();
  }
  ...
  Future<void> handleDelete(BuildContext context, int id) async {
    final _confirmation = await showDialog(
      ...
      builder: (popContext) => InfoDialog.confirmation(
        title: context.t.hrm.confirmations.deleteDepartment,
      ...
      try {
        final _result = await showAsyncLoadingOverlay<String>(
          ...
          asyncFunction: () => Future.microtask(
            () => ref.read(departmentRepoProvider).deleteDepartment(id),
          ),
        );
  ...
  Future<void> handleViewDetails(BuildContext context, DepartmentModel data) async {
        ...
        return BottomModalSheetWrapper(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...[
                  (label: context.t.hrm.department, value: data.name ?? "N/A"),
                  (
  ...
  @override
  Future<DepartmentListModel> fetchData(int page) async {
    return ref
        .read(departmentRepoProvider)
        .getDepartmentList(
          page: page,
  ...
  EventSub<DepartmentModel>? _apiEventSub;
  @override
  void initRefreshListener() {
    _apiEventSub = GlobalEventManager.I.on<DepartmentModel>().listen((_) {
      pagingController.refresh();
    });
